...

{
    ...
    (() =>
    {
        ...
        function doUnwatch(fnName, wuwTarget)
        {
            if (wuwTarget === undefined)
                throw _TypeError(`Argument of ${fnName} is missing or undefined`);
            if (!isNode(wuwTarget))
                throw _TypeError(`Argument of ${fnName} does not implement interface Node`);
        ...
        function doWatch(fnName, wuwTarget)
        {
            if (wuwTarget === undefined)
                throw _TypeError(`Argument of ${fnName} is missing or undefined`);
            if (!isNode(wuwTarget))
        ...
        const returnValue = { doUnwatch, doWatch };
        return returnValue;
    }
    )();

    const wuw = wuwTarget => doWatch('wuw', wuwTarget);

    const watching = { };
    defineConstant(wuw, 'watching', watching);

    {
        const watch = wuwTarget => doWatch('wuw.watch', wuwTarget);
        defineConstant(wuw, 'watch', watch);
    }

    {
        const unwatch = wuwTarget => doUnwatch('wuw.unwatch', wuwTarget);
        defineConstant(wuw, 'unwatch', unwatch);
    }

    {
        const watch = wuwTarget => doWatch('wuw.watching.watch', wuwTarget);
        defineConstant(watching, 'watch', watch);
    }

    {
        const unwatch = wuwTarget => doUnwatch('wuw.watching.unwatch', wuwTarget);
