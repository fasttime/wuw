...

{
    ...
    (() =>
    {
        ...
        if (!_Error_captureStackTrace)
        {
            ...
            (constructorOpt, obj) =>
            {
                constructorOpt.stackTraceOffset = obj.stack.search(/^(?: *at )?0\b/m);
            };
            (
                {
                    0()
...

    {
        ...
        {
            for
            (let slot = firstSlot, record; record = slot.next; slot = recordToSlotMap.get(record))
                yield record;
        };
        ...
        record =>
        {
            lastSlot.next = record;
            lastSlot = { next: null };
            recordToSlotMap.set(record, lastSlot);
            if (logLength < maxLogLength)
                ++logLength;
            else
                firstSlot = recordToSlotMap.get(firstSlot.next);
        };
        ...
        let firstSlot;
        let lastSlot;
        const recordToSlotMap = new WeakMap();
