...

{
    ...
    const { installCallback, notify, uninstallCallback } =
    (() =>
    {
        ...
        const returnValue = { installCallback, notify, uninstallCallback };
        return returnValue;
    }
    )();
    ...
    (() =>
    {
        ...
        function spySet(target, propertyKey, value, receiver)
        {
            ...
            {
                const operation = wuwInfoMap.has(spyInfo.wuwTarget) ? doSpyTargetSet : _Reflect_set;
        ...
        {
            const entries = _Object_entries(ownDescs);
            for (const [propertyKey, ownDesc] of entries)
                _Reflect_defineProperty(wuwTarget, propertyKey, ownDesc);
        }

        function unwatchAs(callerFullName, wuwTarget)
        {
        ...
        function watchAs(callerFullName, wuwTarget)
        {
            if (wuwTarget === undefined)
                throw _TypeError(`Argument of ${callerFullName} is missing or undefined`);
            if (!isNode(wuwTarget))
            ...
            if (!wuwInfoMap.has(wuwTarget))
            {
                ...
                const { proxy, revoke } = _Proxy_revocable(mirror, wuwHandler);
                _Reflect_setPrototypeOf(wuwTarget, proxy);
                wuwInfoMap.set(wuwTarget, { mirror, remarkedPropertyKeys, revoke });
            }
            return wuw;
        }
        ...
        const returnValue = { unwatchAs, watchAs };
        return returnValue;
    }
    )();

    const wuw = wuwTarget => watchAs('wuw', wuwTarget);

    const watching = { };
    defineConstant(wuw, 'watching', watching);

    {
        const watch = wuwTarget => watchAs('wuw.watch', wuwTarget);
        defineConstant(wuw, 'watch', watch);
    }

    {
        const unwatch = wuwTarget => unwatchAs('wuw.unwatch', wuwTarget);
        defineConstant(wuw, 'unwatch', unwatch);
    }

    {
        const watch = wuwTarget => watchAs('wuw.watching.watch', wuwTarget);
        defineConstant(watching, 'watch', watch);
    }

    {
        const unwatch = wuwTarget => unwatchAs('wuw.watching.unwatch', wuwTarget);
        defineConstant(watching, 'unwatch', unwatch);
    }
...

    {
        ...
        defineConstant(watching, 'defaultRemarkUndeletableProperties', remarkUndeletableProperties);
        defineVariable
        (
            watching,
            'remarkUndeletableProperties',
    ...
    function doAs(callerFullName, callback)
    {
    ...
    function dontAs(callerFullName, callback)
    {
        if (callback === undefined)
            throw _TypeError(`Argument of ${callerFullName} is missing or undefined`);
        if (typeof callback !== 'function')
    ...
    const doing = { };
    defineConstant(wuw, 'doing', doing);

    {
        const { do: do_ } = { do: callback => doAs('wuw.do', callback) };
        defineConstant(wuw, 'do', do_);
    }

    {
        const dont = callback => dontAs('wuw.dont', callback);
        defineConstant(wuw, 'dont', dont);
    }

    {
        const { do: do_ } = { do: callback => doAs('wuw.doing.do', callback) };
        defineConstant(doing, 'do', do_);
    }

    {
        const dont = callback => dontAs('wuw.doing.dont', callback);
        defineConstant(doing, 'dont', dont);
    }
